...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.zeros((len(w),self.embed_size * self.n_features))
        for i in range(len(w)):
            x[i] = self.embeddings[w[i]].view(-1)
            #batch = torch.zeros(len(w[i]),self.embed_size)
            #for j in range(len(w[i])):
                #batch[j] = self.embeddings[w[i,j]]
# from here, an important error i made is that when initilizing a 2D tensor, if i do it in the way which  
# torch.zeros(),i only assign the 0-dimension size, which will result in errors.
    ...
    def forward(self, w):
        ...
        features = self.embedding_lookup(w)
        h_drop=self.dropout((nn.ReLU()(features@self.embed_to_hidden_weight + self.embed_to_hidden_bias)))
        logits = h_drop@self.hidden_to_logits_weight + self.hidden_to_logits_bias
