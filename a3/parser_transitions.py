...
#####  for shallow copy, we can change the value of origin by changing the one after copy. depending on
#####  if the thing we want to change is changable. in this case, list is changable, so we can change the 
##### dependencies in partial_parses by changing in minibatch. another thing to mention is that in this function
##### on sentence is finished when it's fully parsed, not just one time as i thought in the beginning.
    ### END YOUR CODE
    Partial_parses = [PartialParse(sentence) for sentence in sentences]
    unfinished_parses = Partial_parses[:]
    while unfinished_parses != []:
        minibatch = unfinished_parses[0:batch_size]
        batch_transitions = model.predict(minibatch)
        for i in range(len(minibatch)):
            minibatch[i].parse_step(batch_transitions[i])
